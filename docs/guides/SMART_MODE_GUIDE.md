...
## 為什麼 Carousel 比 Single Card 畫質更好

Carousel 模式在實際使用中普遍比 single card 更有視覺衝擊力，原因有五：

### 1. 內容密度低
每張 slide 只放 1 個 key_point（headline + subtext）。內容越少，Claude 排版空間越大，字體可以更大、留白更多。

### 2. Role 驅動的設計指令
每張 slide 有明確的 `role`，對應具體的字體大小範圍：

| Role | 設計指令 |
|------|---------|
| `hook` | 40-56px 超大標題，極簡內文，最大視覺衝擊 |
| `data` | 數字 60-96px 主導畫面，強調色，讓數字無法忽視 |
| `point` | 28-36px 粗體標題，14-16px 正文，單一焦點 |
| `quote` | 24-32px 斜體引言，大引號作裝飾元素 |
| `cta` | 粗體指令 + 視覺箭頭，強調色行動元素 |

Single card 沒有這套指令，Claude 自由發揮往往得到平庸排版。

### 3. 尺寸比例更好
B/C 帳號 short body（< 600 字）走 `square`（430×430），正方形縱向空間少，容易擁擠。Carousel 統一用 `story`（430×764），縱向空間充足，視覺層次更豐富。

### 4. 線程位置感知
Prompt 裡告訴 Claude「這是第幾張、共幾張」，它會自然做出節奏感——第一張強衝擊，中間張克制，最後張收尾。

### 5. HTML 驗證修復（2026-04-12）
修復 `_validate_generated_html` 的 fence strip bug：原本末尾空行導致 ` ``` ` 未被正確移除，觸發無效重試。修復後 attempt 1 即可通過，節省 ~50% Haiku token。
