...
| **Sprint 4** | Telegram Bot + Webhook API + PDF/圖片輸入 | 現代內容接入管道 | 規劃中 |
| **Sprint 5** | Web UI 主題選擇器更新（19 主題）+ 輪播圖 / 動態卡片 | 介面對等 + 輸出格式擴展 | 規劃中 |
| **Sprint 6** | 多平台發布 + 定時發布 + 自動化串聯 | 分發閉環 | 規劃中 |
| **Sprint 7** | 互動追蹤 + A/B 測試 + AI 內容洞察 | 智能回饋層 | 規劃中 |
