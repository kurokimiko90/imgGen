...
_VALID_COLOR_MOODS = {
    # Dark themes
    "dark", "dark_tech", "gradient",
    # Light / editorial
    "clean_light", "light", "editorial", "broadsheet", "bulletin_hook",
    # Warm / earthy
    "warm_earth", "warm_sun", "cozy", "paper_data",
    # Bold / contrast
    "bold_contrast", "studio", "luxury_data",
    # Pastel / soft
    "soft_pastel", "pastel",
    # Content-specific
    "data_impact", "ranking", "quote", "quote_dark", "opinion",
    "concept", "trace", "milestone", "gallery_quote", "picks",
    "soft_versus", "hook", "faq", "checklist", "before_after",
}
...
color_mood 判斷標準（從下列選一個最符合的）：
暗色系（科技/深夜感）：dark / dark_tech / gradient
淺色編輯系（清晰易讀）：clean_light / light / editorial / broadsheet / bulletin_hook
溫暖大地系（人文/故事）：warm_earth / warm_sun / cozy / paper_data
強對比系（衝擊/數據）：bold_contrast / studio / luxury_data
柔和粉彩系（輕鬆/創意）：soft_pastel / pastel
內容專屬：data_impact（數據重）/ ranking（排行榜）/ quote / quote_dark（引言）/ opinion（評論）/ concept（概念）/ milestone（里程碑）/ gallery_quote / picks / soft_versus / hook / faq / checklist / before_after / trace

選擇原則：
- 程式碼/工程/系統 → dark_tech 或 dark
- 商業分析/PM/行銷/品牌 → clean_light 或 editorial（預設）
- 人物故事/職涯/溫暖 → warm_earth 或 cozy
- 數據/統計/財務 → data_impact 或 bold_contrast
- 排行榜/Top-N → ranking
- 名言/語錄 → quote 或 quote_dark
- 高端品牌/奢侈 → luxury_data 或 studio
- 新聞報紙風格 → broadsheet 或 bulletin_hook
...
def _validate_smart_data(data: dict[str, Any], cfg: ExtractionConfig) -> None:
    ...
    # Normalize color_mood — fallback to clean_light (not dark_tech)
    if data.get("color_mood") not in _VALID_COLOR_MOODS:
        data["color_mood"] = "clean_light"
